...
fread("/Volumes/MasaHD1/local/NGS_data_analysis/Methylome/emseq/murase/clust2/human_atac.clust.v2.csv")%>%
...
dss <- c(
  WT = "./source/c12_WT_methyl_OpenSites.bedGraph",
  KO1 = "./source/c12_KO1_methyl_OpenSites.bedGraph",
  KO2 = "./source/c12_KO2_methyl_OpenSites.bedGraph"
)
...
bind_rows(prom.mc.all, enh.mc.all) %>%
  ...
  # geom_text(data = stats, aes(y=110, label = sprintf("m = %s\nn=%s", Median, number))) +
...
ggsave(filename = "f5_d.pdf", width = 4, height = 4.8)
